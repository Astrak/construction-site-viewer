...
export default class Zoomer {
  ...
  constructor ( viewer, container ) {
    ...
    const zoomIn = new Button( "<img width=24 src='public/img/zoom-in.svg'/>" );
    zoomIn.domElement.addEventListener( 'click', this.zoomIn.bind( this ), false );
    ...
    this.interactionBox.addEventListener( 'click', this.setZoom.bind( this ), false );
    this.interactionBox.addEventListener( 'mousedown', this.onMouseDown.bind( this ), false );
    this.interactionBox.addEventListener( 'touchstart', this.onMouseDown.bind( this ), false );
    ...
    const zoomOut = new Button( "<img width=24 src='public/img/zoom-out.svg'/>" );
    zoomOut.domElement.style.display = 'block';
    zoomOut.domElement.style.position = 'absolute';
    zoomOut.domElement.style.bottom = '0';
    zoomOut.domElement.addEventListener( 'click', this.zoomOut.bind( this ), false );
  ...
  onMouseMove ( e ) {
    ...
    const y = Math.min( 1, Math.max( 0, getRelativeCoordinates( e, this.interactionBox ).y / parseInt( getComputedStyle( this.interactionBox, null ).height ) ) );
  ...
  update () {
    ...
    const currentZoomScale = ( this.getCurrentZoomDistance() - 3.5 ) / ( 23 - 3.5 );
  ...
  getCurrentZoomDistance () {
    ...
    return this.viewer.camera.position.clone().sub( this.viewer.controls.target ).length();
  ...
  zoomFromTo ( currentZoomDistance, nextZoomDistance ) {
    ...
    this.currentTween = TweenLite.to( tween, 1, {
      ...
      onUpdate () {
        that.viewer.controls.minDistance = that.viewer.controls.maxDistance = tween.distance;
