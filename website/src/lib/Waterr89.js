...
import { Mesh, Color, Vector3, LinearFilter, Matrix4, PerspectiveCamera, RGBFormat, Math as TMath, Vector4, Plane, FrontSide, WebGLRenderTarget, UniformsUtils, UniformsLib, ShaderChunk, ShaderMaterial } from 'three';
...
function Water ( geometry, options ) {
	...
	scope.onBeforeRender = function ( renderer, scene, camera ) {
		...
		scene.traverse( child => {
			child.userData.currentVisibility = child.visible;
			child.visible = child.visible && child.userData.waterMirrored;
		})

		renderer.render( scene, mirrorCamera, renderTarget, true );

		scene.traverse ( child => {
			child.visible = child.userData.currentVisibility;
		})
